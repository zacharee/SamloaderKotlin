...
private class ConstraintSetForInlineDsl(
    ...
    private var handler = CoroutineScope(Dispatchers.Main)
    ...
    override fun isDirty(measurables: List<Measurable>): Boolean {
        ...
        measurables.fastForEachWithIndex { index, measurable ->
 ...
 */
@PublishedApi
internal abstract class EditableJSONLayout(content: String) :
    ...
    private var last = Clock.System.now().run { epochSeconds * 1_000 + nanosecondsOfSecond }
    ...
    override fun setLayoutInformation(information: String) {
        last = Clock.System.now().run { epochSeconds * 1_000 + nanosecondsOfSecond }
...
class State(val density: Density) : SolverState() {
    ...
    init {
        dpToPixel = object : CorePixelDp {
            override fun toPixels(dp: Float): Float {
                return density.density * dp
            }
        }
    }
...
internal open class Measurer : BasicMeasure.Measurer, DesignInfoProvider {
    ...
    override fun measure(widget: ConstraintWidget?, measure: BasicMeasure.Measure) {
        val measurable = widget?.companionWidget ?: return
        ...
        run {
            ...
            obtainConstraints(
                ...
                (measurableLastMeasures?.get(1) ?: 0) == widget.height,
            ...
            obtainConstraints(
                ...
                measure.measureStrategy,
                (measurableLastMeasures?.get(0) ?: 0) == widget.width,
        ...
        if ((measure.measureStrategy == TRY_GIVEN_DIMENSIONS ||
            ...
            val coercedWidth = placeable.width.coerceIn(
                widget.mMatchConstraintMinWidth.takeIf { it > 0 },
                widget.mMatchConstraintMaxWidth.takeIf { it > 0 }
            )
            val coercedHeight = placeable.height.coerceIn(
                widget.mMatchConstraintMinHeight.takeIf { it > 0 },
                widget.mMatchConstraintMaxHeight.takeIf { it > 0 }
            )
        ...
        measure.measuredWidth = currentPlaceable?.width ?: widget.width
        measure.measuredHeight = currentPlaceable?.height ?: widget.height
        val baseline =
            if (currentPlaceable != null && state.isBaselineNeeded(widget)) {
