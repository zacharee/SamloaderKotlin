...
class Entities {
    companion object {
        ...
        fun escape(accum: StringBuilder, string: String, out: OutputSettings, inAttribute: Boolean, normalizeWhite: Boolean, stripLeadingWhite: Boolean) {
                ...
                if (codePoint.code < 0x010000) {
                    ...
                    when (codePoint) {
                        ...
                        '"' -> {
                            ...
                            if (canEncode(c, encoder.charset)) {
                                accum.append(c)
                            } else {
        ...
        private fun appendEncoded(accum: StringBuilder, escapeMode: EscapeMode, codepoint: Char) {
            val name = escapeMode.nameForCodepoint(codepoint)

            if (name != null) {
                accum.append('&').append(name).append(";")
            } else {
                accum.append("&#x").append(codepoint.code.toString(16)).append(";")
            }
        }
        ...
        fun unescape(string: String, strict: Boolean): String {
            return Parser.unescapeEntities(string, string)
        }

        private fun canEncode(c: Char, fallback: Charset): Boolean {
            return when (fallback) {
                Charset.forName("ASCII") -> c.code < 0x80
                Charsets.UTF_8 -> true
                else -> false
            }
        }
    ...
    class EscapeMode private constructor(string: String, size: Int, id: Int) {
        companion object {
            val xhtml = EscapeMode(Entities.xhtml, 4, 0)
            val base = EscapeMode(Entities.base, 106, 1)
            val extended = EscapeMode(Entities.full, 2125, 2)
        ...
        internal val value: Int = id
        internal val entitiesByName = ArrayList<NamedCodepoint>()
        internal val entitiesByCodepoint by lazy { entitiesByName.sortedBy { it.scalar } }

        init {
            val reader = CharacterReader(string)
            ...
            while (!reader.isEmpty) {
                ...
                reader.advance()
                val cp2 = if (codeDelim == ',') {
                    (reader.consumeTo(';').toIntOrNull(codepointRadix) ?: 0).also {
                        reader.advance()
                    }
                } else {
                    empty
                ...
                entitiesByName.add(NamedCodepoint(cp1, name))

                if (cp2 != empty) {
                    multipoints[name] = charArrayOf(cp1.toChar(), cp2.toChar())
                }
            }
        ...
        fun codepointForName(name: String): Char? {
            ...
            val entity = entitiesByName.getOrNull(ix) ?: return null
            if (entity.name != name) return null

            return entity.scalar.toChar()
        }

        fun nameForCodepoint(codepoint: Char): String? {
            var ix = entitiesByCodepoint.binarySearch { it.scalar.compareTo(codepoint.code) }
            val matches = ArrayList<String>()

            while (ix < entitiesByCodepoint.size && entitiesByCodepoint[ix].scalar.toChar() == codepoint) {
                matches.add(entitiesByCodepoint[ix].name)
                ix++
            }
            return if (matches.isEmpty()) null else matches.maxOrNull()
        }
