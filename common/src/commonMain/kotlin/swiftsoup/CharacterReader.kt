...
class CharacterReader(private val input: String) {
    ...
    val isEmpty = pos >= input.length
    val current = if (isEmpty) EOF else input[pos]
    ...
    fun nextIndexOf(c: Char): Int {
        return input.substring(pos).indexOf(c)
    }
    ...
    fun nextIndexOf(seq: String): Int {
        ...
        MATCH@
        while (true) {
            val firstCharIx = input.substring(start).indexOf(firstChar)
