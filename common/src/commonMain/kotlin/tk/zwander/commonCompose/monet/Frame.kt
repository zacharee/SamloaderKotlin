...
class Frame private constructor(
    ...
    companion object {
        ...
        val DEFAULT = make(
            ...
            (200.0f / PI * CamUtils.yFromLstar(50.0) / 100f).toFloat(),
            50.0f,
            2.0f,
            false
        )
        ...
        fun make(
            whitepoint: FloatArray, adaptingLuminance: Float,
            backgroundLstar: Float, surround: Float, discountingIlluminant: Boolean
        ): Frame {
            ...
            var d =
                if (discountingIlluminant) 1.0f else f * (1.0f - 1.0f / 3.6f * exp(
            ...
            val rgbD = floatArrayOf(
                d * (100.0f / rW) + 1.0f - d, d * (100.0f / gW) + 1.0f - d,
                d * (100.0f / bW) + 1.0f - d
            )
            ...
            val k4 = k * k * k * k
            val k4F = 1.0f - k4
            val fl: Float = k4 * adaptingLuminance + 0.1f * k4F * k4F * cbrt(
            ...
            val z: Float = 1.48f + sqrt(n.toDouble()).toFloat()

            // Luminance-level induction factors
            val nbb: Float = 0.725f / pow(n.toDouble(), 0.2).toFloat()
            ...
            val rgbAFactors = floatArrayOf(
                pow(fl * rgbD[0] * rW / 100.0, 0.42).toFloat(),
                pow(fl * rgbD[1] * gW / 100.0, 0.42).toFloat(),
                pow(
